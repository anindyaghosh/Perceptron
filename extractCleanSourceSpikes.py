...
class extractSourceSpikes:
    def __init__(self, filename, new_resolution, version, _noise_thr=0):
        self.filename = filename
        self.original_resolution = np.array([640, 480])
        self.new_resolution = new_resolution
        self.version = version
        self.timesteps_per_frame = 33
        self._noise_thr = _noise_thr
        
    def path(self):
        outDir = 'polarity_add33' if self.version == 'v1' else 'polarity_add'
        if 'do' in self.filename:
            dirFolder = 'do_dataset'
        elif 'dx' in self.filename:
            dirFolder = 'dvx_dataset'
        elif 'events' in self.filename:
            dirFolder = 'do_dataset'
        else:
            dirFolder = 'dvx_dataset'
        self.Path = os.path.join(root, dirFolder, outDir, self.filename)
    ...
    def noise_threshold(self):
        if self.version == 'naive':
            ...
            reshape_scale = 1 # np.sqrt(self.scale[0] * self.scale[1])
            self.noise_thr = np.ceil(reshape_scale / 4)
        elif self.version == 'v2':
            self.noise_thr = self._noise_thr # self.scale[0] * self.scale[1] * (100 / 27.5) * 0.225 * 0.05
            
        # Spatially segregated amacrine cells for ON and OFF pathways
        # Zhang C, Yu WQ, Hoshino A, Huang J, Rieke F, Reh TA, Wong ROL. 
        # Development of ON and OFF cholinergic amacrine cells in the human fetal retina. J Comp Neurol. 
        # 2019 Jan 1;527(1):174-186. doi: 10.1002/cne.24405. Epub 2018 Feb 25. PMID: 29405294; PMCID: PMC6078822.
...
# V1  
    ...
    def timestep_spikes(self, frames):
        ...
        timestep_inputs = np.zeros((self.new_resolution[0] * self.new_resolution[1], 1, num_frames), np.float32)
        timestep_adj = np.zeros((self.new_resolution[0] * self.new_resolution[1], 1, num_frames-1), np.float32)
        for i, f in frames:
            timestep_inputs[:,:,i] += f
...
# ----------------------------------------------------------------------------
    ...
    def read_input_spikes(self):
        ...
        self.frame_spike = np.zeros((self.new_resolution[1], self.new_resolution[0]), np.float32)
